...
void LAppView::Render()
{
    ...
    _drag->SetWindowSize(maxWidth, maxHeight);
...
void LAppView::InitializeSprite()
{
    ...
    imageName = DragImageName;
    LAppTextureManager::TextureInfo* dragTexture = textureManager->CreateTextureFromPngFile(resourcesPath + imageName);

    // 保持左侧位置但缩小尺寸
    x = static_cast<float>(dragTexture->width * 0.5f);
    y = static_cast<float>(dragTexture->height * 0.5f);
    fWidth = static_cast<float>(dragTexture->width * 0.2f); // 缩小到70%
    fHeight = static_cast<float>(dragTexture->height * 0.2f); // 缩小到70%
    _drag = new LAppSprite(x, y, fWidth, fHeight, dragTexture->id, programId);
...
void LAppView::OnTouchesBegan(float px, float py)
{
    ...
    // ドラッグボタンにタップしたか
    if (_drag->IsHit(px, py))
    {
        _isDragging = true;
        int windowPosX, windowPosY;
        GLFWwindow* window = LAppDelegate::GetInstance()->GetWindow();
        glfwGetWindowPos(window, &windowPosX, &windowPosY);
        _touchManager->SetDragStartPosition(static_cast<double>(windowPosX), static_cast<double>(windowPosY));
    }
}
...
void LAppView::OnTouchesMoved(float px, float py)
{
    if (_isDragging)
    {
        ...
        _touchManager->GetDragStartPosition(currentX, currentY);
        double deltaX = px - _touchManager->GetStartX();
        double deltaY = py - _touchManager->GetStartY();
        
        GLFWwindow* window = LAppDelegate::GetInstance()->GetWindow();
        glfwSetWindowPos(window, static_cast<int>(currentX + deltaX), static_cast<int>(currentY + deltaY));
        return;
    }
...
void LAppView::ResizeSprite()
{
    ...
    if (_drag)
    {
        ...
        if (texInfo)
        {
            ...
            fWidth = static_cast<float>(texInfo->width);
            fHeight = static_cast<float>(texInfo->height);
            _drag->ResetRect(x, y, fWidth, fHeight);
        }
    }
}
